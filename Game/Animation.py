...
#         self.sprites = {}
#         self.playing = default_state
#
#     def add(self, name, texture, frames=1, frame_rate=6, facing='right', loop=True, offset=(0, 0)):
#         self.sprites[name] = SingleAnimatedSprite(texture, frames, frame_rate, facing, loop, offset)
#
#     def play(self, name, delta_time):
...
#         self.images = []
#
#         self.original_width = texture.get_width()
#         self.original_height = texture.get_height()
#
#         self.height = self.original_height
#         self.width = self.original_width//frames
#
#         for i in range(frames):
#             self.images.append(texture.subsurface(pygame.Rect(self.width*i, 0, self.width, self.height)))
#
#         self.current_frame = 0
#         self.time_elapsed = 0
#
...
#
#     def next_frame(self, delta_time):
#         if self.loop or self.current_frame < self.frames - 1:
#             self.time_elapsed += delta_time
#             if self.time_elapsed >= self.frame_time:
...
#     def get_frame(self, facing='right'):
#         if self.default_facing != facing:
#             return pygame.transform.flip(self.images[self.current_frame], True, False)
#         return self.images[self.current_frame]
